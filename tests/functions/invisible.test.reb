...
[
    ...
    ('No-Arg is (trap [right-normal |])/id)
    (null? do [right-normal* |])
    (null? do [right-normal*])

    ('No-Arg is (trap [| left-normal])/id)
    (null? do [| left-normal*])
    (null? do [left-normal*])

    ('No-Arg is (trap [right-tight |])/id)
    (null? do [right-tight* |])
    (null? do [right-tight*])

    ('No-Arg is (trap [| left-tight])/id)
    (null? do [| left-tight*])
    (null? do [left-tight*])

    ('No-Arg is (trap [right-soft |])/id)
    (null? do [right-soft* |])
    (null? do [right-soft*])

    ('No-Arg is (trap [| left-soft])/id)
    (null? do [| left-soft*])
    (null? do [left-soft*])
...
(
    x: <unchanged>
    did all [
        'Need-Value is (trap [<discarded> x: ()])/id
        x == <unchanged>
    ]
)(
    x: <unchanged>
    did all [
        'Need-Value is (trap [<discarded> x: comment "hi"])/id
        x == <unchanged>
    ]
)(
    obj: make object! [x: <unchanged>]
    did all [
        'Need-Value is (trap [<discarded> obj/x: comment "hi"])/id
        obj/x == <unchanged>
    ]
)(
    obj: make object! [x: <unchanged>]
    did all [
        'Need-Value is (trap [<discarded> obj/x: ()])/id
        obj/x == <unchanged>
    ]
)

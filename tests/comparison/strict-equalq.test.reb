...
; symmetry
(is? strict-equal? #[bitset! #{00}] #{00} strict-equal? #{00} #[bitset! #{00}])
; email versus text
(
    a-value: to email! ""
...
(
    a-value: to email! ""
    is? strict-equal? to text! a-value a-value strict-equal? a-value to text! a-value
)
(
    a-value: %""
    not strict-equal? a-value to text! a-value
...
(
    a-value: %""
    is? strict-equal? a-value to text! a-value strict-equal? to text! a-value a-value
)
(not strict-equal? #{00} #[image! [1x1 #{00}]])
; symmetry
(is? strict-equal? #{00} #[image! [1x1 #{00}]] strict-equal? #[image! [1x1 #{00}]] #{00})
(not strict-equal? #{00} to integer! #{00})
; symmetry
(is? strict-equal? #{00} to integer! #{00} strict-equal? to integer! #{00} #{00})
(
    a-value: #a
    not strict-equal? a-value to text! a-value
)
...
; symmetry
(is? strict-equal? #{} blank strict-equal? blank #{})
(
    a-value: ""
...
(
    a-value: ""
    is? strict-equal? a-value to binary! a-value strict-equal? to binary! a-value a-value
)
(
    a-value: to tag! ""
    not strict-equal? a-value to text! a-value
...
(
    a-value: 0.0.0.0
    is? strict-equal? to binary! a-value a-value strict-equal? a-value to binary! a-value
)
...
(
    a-value: first ['a/b]
    parse :a-value [b-value:]
    is? strict-equal? :a-value :b-value strict-equal? :b-value :a-value
)
(not strict-equal? [] blank)
; symmetry
(is? strict-equal? [] blank strict-equal? blank [])
...
; symmetry
(is? strict-equal? any-number! integer! strict-equal? integer! any-number!)
; reflexivity
(strict-equal? -1 -1)
; reflexivity
(strict-equal? 0 0)
...
; symmetry
(
    is? strict-equal? to decimal! #{3FD3333333333333} to decimal! #{3FD3333333333334}
...
; symmetry
(
    is? strict-equal? to decimal! #{3FB9999999999999} to decimal! #{3FB999999999999A}
...
(not strict-equal? $0 0%)
; symmetry
(is? strict-equal? 1 1.0 strict-equal? 1.0 1)
; symmetry
(is? strict-equal? 1 $1 strict-equal? $1 1)
; symmetry
(is? strict-equal? 1 100% strict-equal? 100% 1)
; symmetry
(is? strict-equal? 1.0 $1 strict-equal? $1 1.0)
; symmetry
(is? strict-equal? 1.0 100% strict-equal? 100% 1.0)
; symmetry
(is? strict-equal? $1 100% strict-equal? 100% $1)
; approximate equality
(not strict-equal? 10% + 10% + 10% 30%)
; symmetry
(is? strict-equal? 10% + 10% + 10% 30% strict-equal? 30% 10% + 10% + 10%)
; date!; approximate equality
(not strict-equal? 2-Jul-2009 2-Jul-2009/22:20)
; symmetry
(is? strict-equal? 2-Jul-2009 2-Jul-2009/22:20 strict-equal? 2-Jul-2009/22:20 2-Jul-2009)
; missing time = 00:00:00+00:00, by time compatibility standards
(not strict-equal? 2-Jul-2009 2-Jul-2009/00:00:00+00:00)
; symmetry
(is? strict-equal? 2-Jul-2009 2-Jul-2009/00:00 strict-equal? 2-Jul-2009/00:00 2-Jul-2009)
; no timezone math in date!
(not strict-equal? 2-Jul-2009/22:20 2-Jul-2009/20:20-2:00)
; time!
(strict-equal? 00:00 00:00)
; char!; symmetry
(is? strict-equal? #"a" 97 strict-equal? 97 #"a")
; symmetry
(is? strict-equal? #"a" 97.0 strict-equal? 97.0 #"a")
; case
...
(not strict-equal? 'a 'A)
; symmetry
(is? strict-equal? 'a 'A strict-equal? 'A 'a)
; binding not checked by STRICT-EQUAL? in Ren-C (only casing and type)
(strict-equal? 'a use [a] ['a])
; symmetry
(is? strict-equal? 'a use [a] ['a] strict-equal? use [a] ['a] 'a)
; different word types
(not strict-equal? 'a first [:a])
; symmetry
(is? strict-equal? 'a first [:a] strict-equal? first [:a] 'a)
; different word types
(not strict-equal? 'a first ['a])
; symmetry
(is? strict-equal? 'a first ['a] strict-equal? first ['a] 'a)
...
; symmetry
(is? strict-equal? first [:a] first ['a] strict-equal? first ['a] first [:a])
; different word types
(not strict-equal? first [:a] /a)
; symmetry
(is? strict-equal? first [:a] /a strict-equal? /a first [:a])
; different word types
(not strict-equal? first [:a] first [a:])
; symmetry
(is? strict-equal? first [:a] first [a:] strict-equal? first [a:] first [:a])

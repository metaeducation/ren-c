...
[
    ...
    ('a == parse [a a b a] [<any> to 'b 2 <any>])
    (didn't parse [a] [to ['a]])
    (didn't parse [a] [to ['a] <end>])
    ('a == parse [a] [to ['a] <any>])
    ('b == parse [a b] [to ['b] <any>])
    ('b == parse [a a a b] [to ['b] <any>])
    ('a == parse [a a b a] [<any> to ['b] 2 <any>])
    ('c == parse [z z a b c] [to ['c | 'b | 'a] 3 <any>])
    ('c == parse [z z a b c] [to ['a | 'b | 'c] 3 <any>])
    (didn't parse [] [to 'a])
    (didn't parse [] [to ['a]])
]
...
[
    ...
    ("" == parse "aaab" [to [#a] to <end>])
    (#b == parse "ab" [to [#a] 2 <any>])
    (#c == parse "zzabc" [to [#c | #b | #a] 3 <any>])
    (#c == parse "zzabc" [to [#a | #b | #c] 3 <any>])
    (didn't parse "" [to "a"])
    (didn't parse "" [to #a])
    (didn't parse "" [to ["a"]])
    (didn't parse "" [to [#a]])
...
[https://github.com/red/red/issues/2818
    (#c == parse "abc" [to [s: <here> "bc"] 2 <any>])
    (#c == parse "abc" [to [s: <here> () "bc"] 2 <any>])
    (#c == parse "abc" [to [s: <here> (123) "bc"] 2 <any>])
]
...
[
    ...
    (11 == parse #{0A0B} [to #{0A} 2 <any>])
    (#{} == parse #{0A0A0A0B} [to #{0A} to <end>])
    (didn't parse #{0A} [to [#{0A}]])
    (didn't parse #{0A} [to [#{0A}] <end>])
    (10 == parse #{0A} [to [#{0A}] <any>])
    (11 == parse #{0A0B} [to [#{0A}] 2 <any>])
    (12 == parse #{99990A0B0C} [to [#"^L" | #{0B} | #{0A}] 3 <any>])
    (12 == parse #{99990A0B0C} [to [#{0A} | #{0B} | #"^L"] 3 <any>])
    (#{} == parse #{0A0A0A0B} [to [#{0A}] to <end>])
    (didn't parse #{} [to #{0A}])
    (didn't parse #{} [to #"^/"])
    (didn't parse #{} [to [#{0A}]])

...
[
    ...
    ('c == parse [z z a b c] [thru ['c | 'b | 'a] 2 <any>])
    ('c == parse [z z a b c] [thru ['a | 'b | 'c] 2 <any>])
    ('c == parse [b b a a c] [thru 2 'a 'c])
    ('c == parse [b b a a c] [thru 2 'a 'c])
    ('c == parse [b b a a c] [thru [2 'a] 'c])
    ('c == parse [b b a a c] [thru some 'a 'c])
    ('c == parse [b b a a c] [thru [some 'a] 'c])
    ('c == parse [b b a a c] [thru [some 'x | 2 'a] 'c])
    ('c == parse [b b a a c] [thru 2 wa 'c])
...
[
    ...
    (#a == parse "aaba" [<any> thru #a 2 <any>])
    (#a == parse "a" [thru [#a]])
    ("" == parse "a" [thru [#a] <end>])
    (didn't parse "a" [thru [#a] <any>])
    (#b == parse "ab" [thru [#a] <any>])
    (#a == parse "aaba" [<any> thru [#a] 2 <any>])
    (#c == parse "zzabc" [thru [#c | #b | #a] 2 <any>])
    (#c == parse "zzabc" [thru [#a | #b | #c] 2 <any>])
    (#c == parse "bbaaac" [thru 3 #a #c])
    ("c" == parse "bbaaac" [thru 3 "a" "c"])
    (
        wa: [#a]
        #c == parse "bbaaac" [thru 3 wa #c]
    )
    ("c" == parse "bbaaac" [thru [3 "a"] "c"])
...
[
    ...
    (10 == parse #{0A0A0B0A} [<any> thru #{0A} 2 <any>])
    (#{0A} == parse #{0A} [thru [#{0A}]])
    (#{} == parse #{0A} [thru [#{0A}] <end>])
    (didn't parse #{0A} [thru [#{0A}] <any>])
    (11 == parse #{0A0B} [thru [#{0A}] <any>])
    (10 == parse #{0A0A0B0A} [<any> thru [#{0A}] 2 <any>])
    (12 == parse #{99990A0B0C} [thru [#"^L" | #{0B} | #{0A}] 2 <any>])
    (12 == parse #{99990A0B0C} [thru [#{0A} | #{0B} | #"^L"] 2 <any>])
    (#"^L" == parse #{0B0B0A0A0A0C} [thru 3 #{0A} #"^L"])
    (#{0C} == parse #{0B0B0A0A0A0C} [thru 3 #{0A} #{0C}])
    (#"^L" == parse #{0B0B0A0A0A0C} [thru 3 wa #"^L"])
    (#{0C} == parse #{0B0B0A0A0A0C} [thru [3 #{0A}] #{0C}])

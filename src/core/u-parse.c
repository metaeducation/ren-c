...
static bool Subparse_Throws(
    ...
    //
    Finalize_Nihil(ARG(num_quotes));
    Finalize_Nihil(ARG(position));
    Finalize_Nihil(ARG(save));
...
//
{
    ...
    //
    assert(Is_Nihil(ARG(num_quotes)));
    ...
    Cell(const*) set_or_copy_word = NULL;
...
// https://forum.rebol.info/t/1084
{
    ...
    Cell(const*) rules_tail;
    Cell(const*) rules_at = VAL_ARRAY_AT(&rules_tail, rules);

...
static Result(None) Reuse_Sublevel_For_Intrinsic_Arg_Core(
    Level* L
){
    Level* sub = SUBLEVEL;
    assert(sub->executor == &Skip_Me_Executor);
...
#define Reuse_Sublevel_For_Intrinsic_Arg() \
    Reuse_Sublevel_For_Intrinsic_Arg_Core(L)
...
Option(Phase*) Reuse_Sublevel_To_Determine_Left_Literal_Infix_Core(
  ...
  // Put OUT back in CURRENT and CURRENT back in feed
    ...
    Track_Unshield_Cell(&L->feed->fetched);
    Move_Cell(&L->feed->fetched, CURRENT);
    Track_Shield_Cell(&L->feed->fetched);
...
Bounce Stepper_Executor(Level* L)
{
  ...
  // by whatever processing that we do.
    ...
    require (  // for the sake of simpler invariants, always push a Level
      Level* sub = Make_Level(
        ...
        LEVEL_MASK_NONE  // flags will be rewritten
      )
    );
    Push_Level(sub);
  ...
  //    with TYPE_BEDROCK_HOLE...it *acts* like a NULL, but is "beneath" null
    ...
    if (
        ...
        switch (Parameter_Class(param)) {
          ...
          case PARAMCLASS_LITERAL:
            The_Next_In_Feed(Level_Out(SUBLEVEL), L->feed);
            goto intrinsic_arg_in_subout;
        ...
        require (
          Reuse_Sublevel_For_Intrinsic_Arg()
        );

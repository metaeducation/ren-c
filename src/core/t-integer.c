...
// Functions we have to define due to MBEDTLS_HOOK_BIGNUM.
// These are all the routines that change the data pointer or size.
// See documentation for these functions in bignum.h (not repeated here)

void mbedtls_mpi_init(mbedtls_mpi *X) {  // see bignum.h
    assert(X != nullptr);

    X->s = 1;
    X->n = 0;
    X->p = nullptr;
    TRASH_POINTER_IF_DEBUG(X->hookdata);
}

void mbedtls_mpi_free(mbedtls_mpi *X) {  // see bignum.h
    if (X == nullptr)
        return;

    if (X->p == nullptr) {
        assert(IS_POINTER_TRASH_DEBUG(X->hookdata));
        return;
    }

    REBBIG *big = SER(X->hookdata);
    if (GET_SERIES_FLAG(big, MANAGED)) {
        //
        // Once the series is managed (usually by virtue of being put into
        // an INTEGER! cell), we can't forcibly free it.  Just let it GC.
    }
    else {
        // An unmanaged bignum would be one generated by the cryptography
        // library for an internal operation.  We could have promoted it to
        // managed and then used it in an INTEGER!, but we didn't, so free.
        //
        Free_Unmanaged_Series(big);
    }
...
int mbedtls_mpi_grow(mbedtls_mpi *X, size_t nblimbs) {  // see bignum.h
    assert(X != nullptr);
    ...
    if (X->p == nullptr) {
        ...
        REBSER *big = Make_Series(nblimbs, sizeof(mbedtls_mpi_uint));

        X->p = SER_HEAD(mbedtls_mpi_uint, big);
        memset(X->p, 0, X->n * sizeof(mbedtls_mpi_uint));  // they use calloc

        X->n = nblimbs;
        SET_SERIES_USED(big, nblimbs);

        X->hookdata = big;
        return 0;
    }

    REBSER *big = SER(X->hookdata);
    assert(X->p == SER_HEAD(mbedtls_mpi_uint, big));
    assert(X->n == SER_USED(big));

    if (SER_REST(big) < nblimbs) {  // needs expansion
        EXPAND_SERIES_TAIL(big, nblimbs - X->n);
        X->p = SER_HEAD(mbedtls_mpi_uint, big);  // refresh pointer
    }

    // force the expanded space to be all zeros
    //
    memset(X->p + X->n, 0, sizeof(mbedtls_mpi_uint) * (nblimbs - X->n));
    SET_SERIES_USED(big, nblimbs);
    X->n = nblimbs;
...
int mbedtls_mpi_shrink(mbedtls_mpi *X, size_t nblimbs) {  // see bignum.h
    assert(X != nullptr);
    ...
    // Prune out as many zeroed limbs as there are.
    ...
    // !!! This is where we would shrink for efficiency.  Rebol needs work on
    // reclaiming space in general--look at a systemic answer before fixing
    // up bignums on this particular point.

    SET_SERIES_USED(big, i);
    X->n = i;
    X->p = SER_HEAD(mbedtls_mpi_uint, big);
...
REBTYPE(Integer)
{
    ...
    case SYM_ADD: {
        REBI64 anum;
        if (REB_INTPTR_ADD_OF(num, arg, &anum))
            fail (Error_Overflow_Raw());
        return Init_Integer(D_OUT, anum); }

    case SYM_SUBTRACT: {
        REBI64 anum;
        if (REB_INTPTR_SUB_OF(num, arg, &anum))
            fail (Error_Overflow_Raw());
        return Init_Integer(D_OUT, anum); }

    case SYM_MULTIPLY: {
        REBI64 p;
        if (REB_INTPTR_MUL_OF(num, arg, &p))
            fail (Error_Overflow_Raw());
    ...
    case SYM_ROUND: {
        ...
        USED(ARG(value));  // extracted as num, others are passed via frame_

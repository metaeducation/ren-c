...
void Rebind_Context_Deep(
    ...
    const Cell *tail = ARR_TAIL(CTX_VARLIST(dest));
...
//
REBARR *Context_To_Array(const Cell *context, REBINT mode)
{
    assert(!(mode & 4));
...
//
REBVAL *Select_Symbol_In_Context(const Cell *context, Symbol(const*) symbol)
{

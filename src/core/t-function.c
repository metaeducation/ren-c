...
***********************************************************************/
{
	...
	types = Copy_List_At_Shallow(types, 1);
...
***********************************************************************/
{
	...
	case A_REFLECT:
		...
		case OF_BODY:
			...
			case REB_FUNCTION:
				Val_Init_Block(
					D_OUT, Copy_List_Deep_Managed(VAL_FUNC_BODY(value))
				);
		...
		case OF_SPEC:
			Val_Init_Block(
				value, Copy_List_Deep_Managed(VAL_FUNC_SPEC(value))
			);
		...
		case OF_TITLE:
			...
			Val_Init_String(value, Copy_Array(VAL_SERIES(arg)));
			break;
		default:
		bad_arg:

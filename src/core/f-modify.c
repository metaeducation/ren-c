    ...
    if (flags & AM_SPLICE) {
        ...
        if (dst_arr == Cell_Array(src_val)) {
            Array* copy = Copy_Array_At_Extra_Shallow(
                ...
                0, // extra
                BASE_FLAG_MANAGED // !!! Worth it to not manage and free?
            );
    ...
    if (Is_Integer(src_val)) {
        ...
        *Binary_Head(src_ser) = cast(Byte, i);
        ...
        if (limit >= 0 && len_at > cast(REBLEN, limit))
            src_ser = Make_Utf8_From_String_At_Limit(src_val, limit);
        else
            src_ser = Make_Utf8_From_String_At_Limit(src_val, len_at);
        needs_free = true;
        limit = -1;
    }

...
Bounce MAKE_Pair(Value* out, Type type, const Value* arg)
{
    ...
    if (Is_Text(arg)) {
        ...
        //
        Size size;
        Byte *bp = Analyze_String_For_Scan(&size, arg, Series_Len_At(arg));
...
REBTYPE(Pair)
{
   ...
   // !!! The only way we can generically guarantee the ability to retrigger
    ...
    Value* x = rebValue(NAT_VALUE(EVALUATE), rebR(x_frame));
    Value* y = rebValue(NAT_VALUE(EVALUATE), rebR(y_frame));

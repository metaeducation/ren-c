...
//=//// CELL_FLAG_OUT_HINT_UNSURPRISING ///////////////////////////////////=//
//
// This is a flag that the action executor may set on its output cell.  It's
// in the negative sense on purpose...!
//
// It exists to address an issue with functions that produce GHOST! or ACTION!
// values, when they don't produce them *always*.  For instance:
//
//      result: case [
//          condition1 [mode: <no-update> ...]
//          condition2 [mode: <update> eval code]
//      ]
//
// If EVAL can produce a GHOST!, there's a danger that result will be the
// <update> tag from the prior expression, instead of the EVAL result.
//
// Similar issues arise with ACTION! values, since if a function sometimes
// returns an ACTION! and sometimes does not, it's difficult to handle:
//
//     result: if true [
//         item: select obj 'field
//         probe type of item  ; if ITEM is an action, it's active/runs
//         [a b c]  ; this could act as an argument to the item function!
//     ]
//
// We could add a refinement to EVAL such as :GHOSTABLE to approve ghost
// return results (otherwise it could error or evaluate to VOID).  And SELECT
// could have a refinement like :ACTIVE to approve ACTION! results.  But these
// refinements would have to be added to any function fitting this pattern.
//
// So instead, we make note of when a function returns GHOST! or ACTION!
// and its type signature says that's the only thing it does.  In this case,
// we label the cell as being CELL_FLAG_OUT_HINT_UNSURPRISING.  Callers then
// decide whether to approve the result or not.  By having the flag default
// to negative, it makes it harder for code paths to accidentally implicitly
// approve surprising results--the bias is to be explicit about when it is
// specifically *not* surprising.
//
// The UNAFRAID operator (^) is used to approve surprising results.  This
// allows surprising GHOST!s to vanish, and also allows ACTION! to be used in
// plain SET-WORD assignments despite being surprising.  (Lifted assignments
// accept surprising results, but they are not the same as set-word.)
//
#define CELL_FLAG_OUT_HINT_UNSURPRISING  CELL_FLAG_HINT

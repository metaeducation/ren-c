...
INLINE bool Is_Block_Style_Varargs(
    ...
    if (vararg->extra.binding->leader.bits & ARRAY_FLAG_IS_VARLIST) {
...
INLINE bool Is_Level_Style_Varargs_Maybe_Null(
    ...
    if (not (vararg->extra.binding->leader.bits & ARRAY_FLAG_IS_VARLIST)) {
...
INLINE const Value* Param_For_Varargs_Maybe_Null(const Cell* v) {
    ...
    //
    assert(not (v->extra.binding->leader.bits & ARRAY_FLAG_IS_VARLIST));
    return nullptr;
}

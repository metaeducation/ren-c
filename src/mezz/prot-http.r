REBOL [
    ...
    History: {modified to return an error object with the info object when manual redirect required - Graham}
    Date: 8-Oct-2015
]
...
sync-op: func [port body /local state ] [
    ...
    while [not find [ready close] state/state][
        unless port? wait [state/connection port/spec/timeout] [fail make-http-error "Timeout"]
    ...
    actor: [
        read: func [
            ...
            either any-function? :port/awake [
                ...
                unless port/state/state = 'ready [fail make-http-error "Port not ready"]
        ...
        open: func [
            ...
            unless port/spec/host [fail make-http-error "Missing host address"]
            port/state: context [
                state: 'inited
                connection:
                error: none
